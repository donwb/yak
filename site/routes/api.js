...
router.get('/yaksForArea/:area', function(req, res) {
	...
	if(isNaN(area)) {
		...
		runQuery("region", area, function(returnValue) {
...
router.get('/yaksForUser/:userId', function(req, res){
	...
	if(isNaN(user)) {
		...
		runQuery("userid", user, function(returnValue) {
			res.send(returnValue);
		})
	}
...
function runQuery(field, value, callback) {
	...
	db.yaks.find(query).toArray(function(err, items){
		console.log(items);
		if(items.length === 0) {
